...
router.get('/storage', requireAuth, requirePermission('MANAGE_PLATFORM'), (req, res) => {
  const total = db.prepare(`SELECT COALESCE(SUM(so.size), 0) AS bytes, COUNT(*) AS objects FROM storage_objects so`).get();
  const attached = db.prepare(`SELECT COUNT(*) AS n FROM attachments`).get().n;
