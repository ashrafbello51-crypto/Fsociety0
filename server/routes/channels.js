...
function channelCommunity(channelId) {
...
router.get('/:channelId/messages', requireAuth, (req, res) => {
  ...
  const ch = channelCommunity(channelId);
  if (!ch) return res.status(404).json({ error: 'Channel not found' });
  if (!isMember(ch.community_id, req.user.id)) return res.status(403).json({ error: 'Forbidden' });
  ...
  if (ids.length) {
    ...
    const reacts = db
      .prepare(`SELECT message_id, emoji, COUNT(*) AS count, SUM(user_id = ?) AS mine FROM message_reactions WHERE message_id IN (${placeholders}) GROUP BY message_id, emoji`)
      .all(req.user.id, ...ids);
    reacts.forEach((r) => {
...
router.post('/:channelId/messages', requireAuth, MESSAGE_LIMITER, (req, res) => {
  const channelId = parseInt(req.params.channelId, 10);
  const ch = channelCommunity(channelId);
  if (!ch) return res.status(404).json({ error: 'Channel not found' });
  if (!isMember(ch.community_id, req.user.id)) return res.status(403).json({ error: 'Forbidden' });
  ...
  try {
    messageId = db.tx((d) => {
      const r = d.prepare(`INSERT INTO messages (channel_id, author_id, body) VALUES (?, ?, ?)`).run(channelId, req.user.id, parsed.data.body || '');
