...
router.post('/', requireAuth, async (req, res) => {
  ...
  try {
    const r = await db.prepare(`INSERT INTO communities (name, slug, description, owner_id) VALUES (?, ?, ?, ?)`).run(
      parsed.data.name, slug, parsed.data.description || '', req.user.id
    );
    const cid = r.lastInsertRowid;
    await db.prepare(`INSERT INTO community_members (community_id, user_id, role) VALUES (?, ?, 'owner')`).run(cid, req.user.id);
...
router.post('/:id/join', requireAuth, async (req, res) => {
  ...
  const exists = await db.prepare(`SELECT id FROM communities WHERE id = ?`).get(id);
  if (!exists) return res.status(404).json({ error: 'Community not found' });
  if (await isMember(id, req.user.id)) return res.json({ ok: true });
  await db.prepare(`INSERT OR IGNORE INTO community_members (community_id, user_id, role) VALUES (?, ?, 'member')`).run(id, req.user.id);
  res.json({ ok: true });
});

router.post('/:id/leave', requireAuth, async (req, res) => {
  const id = parseInt(req.params.id, 10);
  await db.prepare(`DELETE FROM community_members WHERE community_id = ? AND user_id = ?`).run(id, req.user.id);
  res.json({ ok: true });
});
