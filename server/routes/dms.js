...
async function shareCommunity(a, b) {
  return db.prepare(
...
async function getPresence(userId) {
  const row = await db.prepare(`SELECT state, last_seen FROM presence WHERE user_id = ?`).get(userId);
...
async function buildConversation(conversationId, viewerId) {
  ...
  const presence = await getPresence(other.id);
...
router.get('/', requireAuth, async (req, res) => {
  ...
  const list = (await Promise.all(rows.map((r) => buildConversation(r.id, req.user.id)))).filter(Boolean);
...
router.post('/', requireAuth, async (req, res) => {
  ...
  if (!(await shareCommunity(req.user.id, targetId))) {
  ...
  if (existing) {
    ...
    const r = await db.prepare(`INSERT INTO conversations DEFAULT VALUES`).run();
    conversationId = r.lastInsertRowid;
    await db.prepare(`INSERT OR IGNORE INTO conversation_members (conversation_id, user_id) VALUES (?, ?)`).run(conversationId, req.user.id);
    await db.prepare(`INSERT OR IGNORE INTO conversation_members (conversation_id, user_id) VALUES (?, ?)`).run(conversationId, targetId);
  }
...
router.get('/:id/messages', requireAuth, async (req, res) => {
  ...
  if (ids.length) {
    ...
    const reacts = await db.prepare(
      `SELECT message_id, emoji, COUNT(*) AS count, SUM((user_id = ?)::int) AS mine
...
router.post('/:id/messages', requireAuth, DM_LIMITER, async (req, res) => {
  const conversationId = parseInt(req.params.id, 10);
  if (!(await memberOf(conversationId, req.user.id))) return res.status(403).json({ error: 'Forbidden' });
  ...
  try {
    messageId = await db.tx(async (d) => {
      const r = await d.prepare(`INSERT INTO messages (conversation_id, author_id, body) VALUES (?, ?, ?)`).run(
        conversationId, req.user.id, parsed.data.body || ''
      );
      const mid = r.lastInsertRowid;
      await d.prepare(`INSERT OR IGNORE INTO message_reads (message_id, user_id) VALUES (?, ?)`).run(mid, req.user.id);
      if (storageRow) {
  ...
  const other = await db.prepare(`SELECT user_id FROM conversation_members WHERE conversation_id = ? AND user_id <> ?`).get(conversationId, req.user.id);
  if (other && !realtime.userInConversation(conversationId, other.user_id)) {
    await notify.createNotification(other.user_id, 'dm', {
      conversationId,
      from: req.user.id,
