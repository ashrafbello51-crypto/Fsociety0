...
async function validTarget(type, id) {
...
async function userSummary(id) {
  if (!id) return null;
  const u = await db.prepare(`SELECT id, username, display_name FROM users WHERE id = ?`).get(id);
  return u ? { id: u.id, username: u.username, displayName: u.display_name } : null;
}

async function targetSummary(type, id) {
  if (type === 'message') {
  ...
  if (type === 'community') {
    const c = await db.prepare(`SELECT id, name FROM communities WHERE id = ?`).get(id);
...
async function serializeReport(row) {
  return {
    id: row.id,
    ...
    reporter: await userSummary(row.reporter_id),
    assigned: await userSummary(row.assigned_to),
    resolvedBy: await userSummary(row.resolved_by),
    target: await targetSummary(row.target_type, row.target_id),
  };
}
...
router.post('/reports', requireAuth, REPORT_LIMITER, async (req, res) => {
  const parsed = reportSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: 'Validation failed', details: parsed.error.flatten() });
  const { targetType, targetId, reason, description } = parsed.data;

  if (!(await validTarget(targetType, targetId))) return res.status(404).json({ error: 'Report target not found' });
...
router.get('/reports', requireAuth, requirePermission('VIEW_REPORTS'), async (req, res) => {
  ...
  res.json({ reports: await Promise.all(rows.map(serializeReport)) });
});

router.get('/reports/:id', requireAuth, requirePermission('VIEW_REPORTS'), async (req, res) => {
  const id = parseInt(req.params.id, 10);
  const row = await db.prepare(`SELECT * FROM reports WHERE id = ?`).get(id);
  if (!row) return res.status(404).json({ error: 'Report not found' });
  res.json(await serializeReport(row));
});

// ---- Claim / assign to self (sets to reviewing) ----
router.post('/reports/:id/claim', requireAuth, requirePermission('VIEW_REPORTS'), async (req, res) => {
  const id = parseInt(req.params.id, 10);
  const row = await db.prepare(`SELECT * FROM reports WHERE id = ?`).get(id);
  if (!row) return res.status(404).json({ error: 'Report not found' });
  await db.prepare(`UPDATE reports SET assigned_to = ?, status = 'reviewing', updated_at = datetime('now') WHERE id = ?`).run(req.user.id, id);
  res.json({ ok: true });
});

// ---- Resolve / dismiss (privileged) ----
async function closeReport(req, res, resolution) {
  const id = parseInt(req.params.id, 10);
  const row = await db.prepare(`SELECT * FROM reports WHERE id = ?`).get(id);
  if (!row) return res.status(404).json({ error: 'Report not found' });
...
router.post('/users/:id/moderate', requireAuth, requirePermission('MANAGE_USERS'), ACTION_LIMITER, async (req, res) => {
  ...
  const target = await db.prepare(`SELECT id, username, status FROM users WHERE id = ?`).get(targetId);
  if (!target) return res.status(404).json({ error: 'User not found' });

  // Never allow moderating a privileged account (any user holding MANAGE_USERS).
  if (await userHasPermission(targetId, 'MANAGE_USERS')) {
  ...
  await db.tx(async (d) => {
    await d.prepare(`UPDATE users SET status = ?, expires_at = ?, status_note = ? WHERE id = ?`).run(
    ...
    if (isRestrictive) await d.prepare(`DELETE FROM sessions WHERE user_id = ?`).run(targetId);
  });
  ...
  if (isRestrictive || action === 'warn') {
    await notify.createNotification(targetId, 'moderation', {
...
router.get('/audit', requireAuth, requirePermission('VIEW_AUDIT_LOGS'), async (req, res) => {
