...
function authorizeAttachment(user, att) {
  if (att.channel_id) {
    const ch = db.prepare(`SELECT community_id FROM channels WHERE id = ?`).get(att.channel_id);
    if (!ch) return false;
    return isMember(ch.community_id, user.id);
  }
  if (att.conversation_id) {
    return !!db.prepare(`SELECT 1 FROM conversation_members WHERE conversation_id = ? AND user_id = ?`).get(att.conversation_id, user.id);
  }
  return false;
}
...
router.post('/', requireAuth, uploadLimiter, upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      db.prepare(`DELETE FROM storage_objects WHERE id = ?`).run(r.lastInsertRowid);
      throw saveErr;
    }
    res.status(201).json({
    ...
    if (processed && processed.storageKey) { try { storage.delete(processed.storageKey); } catch {} }
...
router.get('/:id', requireAuth, async (req, res) => {
  ...
  if (!authorizeAttachment(req.user, att)) return res.status(403).json({ error: 'Forbidden' });
...
async function discardStorageObject(storageKey) {
  ...
  db.prepare(`DELETE FROM storage_objects WHERE storage_key = ?`).run(storageKey);
}
...
async function cleanupOrphaned() {
  try {
    ...
    for (const o of orphans) { try { await storage.delete(o.storage_key); } catch {} db.prepare(`DELETE FROM storage_objects WHERE storage_key = ?`).run(o.storage_key); }
    ...
    for (const o of stale) {
      ...
      db.prepare(`DELETE FROM storage_objects WHERE storage_key = ?`).run(o.storage_key);
      db.prepare(`DELETE FROM attachments WHERE storage_key = ?`).run(o.storage_key);
    }
  } catch (e) {
